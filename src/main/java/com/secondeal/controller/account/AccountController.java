    ...
    public Object register() {
        ...
        if (mobile) {
            ...
            try {
                ...
                userActive.setAccount(name);
                userActive.setCount(1L);
                userActive.setCreateTime(StringUtil.getDate());
                int re = userActionI.insert(userActive);
                if (res == 1 && re == 1) {
...
//                        将用户活动表自加一
                        int res = userActionI.update(name);
                        // 返回登录token
                        String token = JavaWebToken.createJavaWebToken(m);
                        m1.put("token", token);
                        m1.put("nickName", nickname);
